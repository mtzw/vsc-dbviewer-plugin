...
v3.4で初期実装した差分機能とv3.4.1の大規模データ対応は、次の順序で完成判定を行います。

1. スナップショット保存・比較・削除、行分類、主キーなし・構成変更、LOB除外、JSON / CSV出力をVS Code実機で受入確認します。
2. 複数チャンク、詳細比較上限、キャンセル、行数・容量超過、50件ページング、フィルター、行詳細を大規模Tableで確認します。
3. v3.4形式との互換読込、破損・欠損データの検出、24時間後の一時データ削除を確認します。
4. H2の自動統合テストを維持し、Microsoft SQL Server、Oracle、PostgreSQL、MySQLの順に実DB確認を拡大します。

全自動テストとビルドの成功、重大度High以上の既知不具合がないこと、実機確認結果の記録をv3.4.2の完了条件とします。
