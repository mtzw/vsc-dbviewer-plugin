...
- v3.1: データタブで選択行を削除できるようにします。主キーで一意に特定できるTableのみを対象にし、削除前プレビュー、確認、トランザクション、失敗時rollbackに対応予定です。
...
- TSV InsertはTableのみ対象です。View、任意SQL実行、行Update、Deleteには対応していません。選択行削除はv3.1で追加予定です。
